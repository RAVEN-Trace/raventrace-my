...
if (pages.length !== 35) throw new Error(`Expected 35 public pages, found ${pages.length}`);
...
async function openDeployed(page, route) {
  for (let attempt = 0; attempt < 60; attempt += 1) {
    ...
    const ready = await page.evaluate(() =>
      ...
      Boolean(document.querySelector('link[href*="raven-unified-v7.css?v=7.0.6"]'))
    );
    if (ready) return url;
    await delay(5000);
  }
...
async function auditPage(page, view, item) {
  ...
  const metrics = await page.evaluate(async ({ mobile }) => {
    ...
    return {
      ...
      unifiedCss: Boolean(document.querySelector('link[href*="raven-unified-v7.css?v=7.0.6"]')),
