...
CURRENT_PUBLICATION = 'raven-publication.js?v=5.0.0'
CURRENT_EVIDENCE = 'raven-evidence-ux.js?v=3.0.0'
...
OLD_EVIDENCE_VERSIONS = [
    'raven-evidence-ux.js?v=2.1.0',
]

PUBLIC_ROOTS = [
    ROOT / 'index.html', ROOT / 'about', ROOT / 'methodology', ROOT / 'corrections',
    ROOT / 'tips', ROOT / 'news', ROOT / 'investigations',
]
...
def public_html_files():
    ...
    for root in PUBLIC_ROOTS:
        files = [root] if root.is_file() else (root.rglob('*.html') if root.exists() else [])
        for path in files:
            if path in seen:
                continue
            seen.add(path)
...
def replace_runtime_versions(text: str) -> str:
    for old in OLD_PUBLICATION_VERSIONS:
        text = text.replace(old, CURRENT_PUBLICATION)
    for old in OLD_EVIDENCE_VERSIONS:
        text = text.replace(old, CURRENT_EVIDENCE)
    return text
...
def patch_home(text: str) -> str:
    text = replace_runtime_versions(text)
...
def patch_html(text: str) -> str:
    return replace_runtime_versions(text)
...
if __name__ == '__main__':
    ...
    for rel in ['assets/js/raven.js', 'assets/js/raven-share.js']:
        path = ROOT / rel
        if path.exists():
            changed += update(path, replace_runtime_versions)
