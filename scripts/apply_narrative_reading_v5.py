...
js = '<script src="/raventrace-my/assets/js/raven-narrative-v5.js?v=5.2.2" defer data-raven-narrative-v5></script>'

# Keep the existing visual CSS; V5.2.2 is a public-language compatibility update.
if css not in text:
...
# Canonicalise any older Narrative V5 JS version to V5.2.2.
text = re.sub(
...
assert out.count('raven-narrative-v5.js?v=5.2.2') == 1
assert 'Cerita yang dibawa' in out
assert 'Apa yang boleh disahkan?' in out
assert ('Raven kata macam mana?' in out) or ('Selepas framing dibuang:' in out)
assert 'Naratif utama' in out
print('Narrative Reading Experience V5.2.2 wiring PASS')
