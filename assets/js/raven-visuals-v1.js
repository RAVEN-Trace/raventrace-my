(() => {
  ...
  const ensureCss=()=>{if(q('link[data-raven-visuals-v1]'))return;const l=document.createElement('link');l.rel='stylesheet';l.href='/raventrace-my/assets/css/raven-visuals-v1.css?v=1.0.0';l.dataset.ravenVisualsV1='true';document.head.appendChild(l)};
  ensureCss();
  ...
  if(path.includes('/investigations/rci-tabung-haji/narratives/')){
    ...
    if(target && !q('[data-raven-visual="narrative"]')){const v=make('/raventrace-my/assets/visuals/raven-narrative-rm13b-v1-web.jpg','Infografik Raven: dakwaan RM13 bilion dicuri dibandingkan dengan rekod RM10.2 bilion beban kerajaan/UJSB dan RM2.6 bilion impairment TH, dengan jurang naratif loss tidak sama dengan theft.','Claim → record → narrative gap → verdict.','narrative');v.dataset.ravenVisual='narrative';target.parentNode.insertBefore(v,target);}
  }
  if(path.includes('/investigations/rci-tabung-haji/money/')){
    const target=q('.case-section');
    if(target && !q('[data-raven-visual="money"]')){const v=make('/raventrace-my/assets/visuals/raven-money-rm13b-v1-web.jpg','Infografik pecahan hampir RM13 bilion: RM10.2 bilion government/UJSB burden dan RM2.6 bilion TH impairments, dengan peringatan bahawa kerugian bukan automatik kecurian.','Pecahan angka rasmi untuk mengelakkan category collapse.','evidence');v.dataset.ravenVisual='money';insertAfter(target.querySelector('h2')||target,v);}
  }
  ...
  if(isCase||isUpdates||isCountStory){
    ...
    if(target && !q('[data-raven-visual="four-five"]')){const v=make('/raventrace-my/assets/visuals/raven-4-vs-5-v1-web.jpg','Infografik perbandingan empat charge-readings yang disahkan melalui rekod mahkamah dengan lima yang dinyatakan oleh SPRM, serta penjelasan bahawa prosiding Adi Azuan ditangguhkan pada cut-off.','Dua rekod awam dipaparkan serentak tanpa menyembunyikan percanggahan.','disputed');v.dataset.ravenVisual='four-five';if(isCase)insertAfter(target.querySelector('h2')||target,v);else target.insertBefore(v,target.firstChild);}
  }
})();
